...
import { deleteRuleLocation, getAllRuleLocation } from "../../services/apis";
import { toast } from "react-toastify";

export const RuleLocation: React.FC = () => {
    ...
    const filteredData = ruleLocationData.filter((item) =>
    ...
    const handleDelete = async () => {
        ...
        try {
            await deleteRuleLocation(deleteId);
            setIsDeleteModalOpen(false);
            setDeleteId(null);
            await fetchRuleLocationData();

            toast.success("Rule location deleted successfully!", {
                position: "top-right",
                autoClose: 3000,
                style: { zIndex: 9999999999, marginTop: "4rem" },
            });
        } catch (error) {
            toast.error("Failed to delete rule location.", {
                position: "top-right",
                autoClose: 3000,
                style: { zIndex: 9999999999, marginTop: "4rem" },
            });
    ...
    return (
        <div className="p-4">
            ...
            <h1 className="text-xl font-bold mb-4">View Rules Location</h1>
            ...
            <div className="relative w-full">
                <button
                    ...
                    onClick={() => navigate("/addRulesLocation")}
                >
                            ...
                            <TableBody className="divide-y divide-gray-100 dark:divide-white/[0.05]">
                                {paginatedItems.map((item, index) => (
                                    <TableRow key={index}>
                                        ...
                                        <TableCell className="px-4 py-3 text-gray-600 text-start">{item.title}</TableCell>
                                        <TableCell className="px-4 py-3 text-gray-600 text-start"> {item?.companyData?.[0]?.name}</TableCell>
                                        ...
                                        <TableCell className="px-4 py-3 text-gray-600 text-start">
                                            <div className="flex gap-2">
                                                <div className="cursor-pointer"
                                                    onClick={() => navigate(`/editRulesLocation/${item._id}`)}>
                ...
                <div className="flex flex-col md:flex-row justify-between items-center mt-4 gap-4">
                    <span className="text-sm">
                        Showing {(currentPage - 1) * itemsPerPage + 1} to {Math.min(currentPage * itemsPerPage, ruleLocationData.length)} of {ruleLocationData.length} entries
                    </span>
                    <Stack spacing={2}>
                        <Pagination
                            count={totalPages}
            ...
            >
                <Box className="fixed inset-0 flex items-center justify-center bg-opacity-50">
                    <div className="bg-white p-6 rounded-lg shadow-lg">
                        ...
                        <p>
                            Are you sure you want to delete this Rule Location?
                        </p>
