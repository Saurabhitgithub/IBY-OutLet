...
export const SubCategory = () => {
  ...
  const [getAllSubCategoryData, setGetAllSubCategoryData] = useState<any[]>([]);
  const [loading, setLoading] = useState(false);
  const [permissions, setPermissions] = useState({
    canViewSubCategories: false,
    canCreateSubCategories: false,
    canEditSubCategories: false,
    canDeleteSubCategories: false,
  });
  ...
  useEffect(() => {
    const initializeComponent = async () => {
      const [
        ...
        checkPermission('Sub-Category', 'View'),
        checkPermission('Sub-Category', 'Create'),
        checkPermission('Sub-Category', 'Update'),
        checkPermission('Sub-Category', 'Delete'),
      ]);

      setPermissions({
        canViewSubCategories: canView,
        canCreateSubCategories: canCreate,
        canEditSubCategories: canEdit,
        canDeleteSubCategories: canDelete,
      });

      if (canView) {
        getAllSubCategoryDataFunction();
      } else {
        toast.error("You don't have permission to view Sub-Category", {
          toastId: "no-permission-view-subcategories",
          position: "top-right",
          autoClose: 3000,
  ...
  const getAllSubCategoryDataFunction = async () => {
    setLoading(true);
    try {
      const response = await getAllSubCategory();
      setGetAllSubCategoryData(response.data.data);
    } catch (error) {
  ...
  const handleToggleStatus = async (item: any) => {
    if (!permissions.canDeleteSubCategories) {
      toast.error("You don't have permission to change subcategory status", {
        position: "top-right",
        autoClose: 3000,
        style: { zIndex: 9999999999, marginTop: "4rem" },
      });
    ...
    try {
      ...
      toast.success(
        `Subcategory ${
          item.active_status ? "deactivated" : "activated"
        } successfully!`,
        {
          position: "top-right",
          autoClose: 3000,
  ...
  const filteredItems = getAllSubCategoryData.filter(
    (item) =>
      searchTerm === "" ||
      item.name.toLowerCase().includes(searchTerm.toLowerCase()) ||
      item.belong_to?.toLowerCase()?.includes(searchTerm.toLowerCase())
  );
  ...
  return (
    <div className="p-4">
      ...
      <h1 className="text-xl font-bold mb-4">Sub-Category</h1>
      ...
      <div className="flex flex-wrap justify-between items-center mb-4 mt-2 gap-2">
        ...
        <div className="flex gap-2">
          {permissions.canCreateSubCategories && (
            <button
              className="bg-brand text-white px-4 py-2 rounded flex items-center gap-1"
      ...
      <div className="relative w-full">
              ...
              {paginatedItems.length > 0 ? (
                paginatedItems.map((item, index) => (
                  ...
                  >
                    ...
                    <TableCell className="px-4 py-3 text-gray-600 text-start">
                      <div className="flex gap-2">
                        {permissions.canEditSubCategories && (
                          <div
                            className="cursor-pointer hover:bg-blue-400 transition-colors"
                            onClick={() =>
                              navigate(`/subcategories/update/${item._id}`)
                            }
                          >
                        ...
                        {permissions.canDeleteSubCategories && (
                          <div
        ...
        <span className="text-sm">
          Showing {(currentPage - 1) * itemsPerPage + 1} to {Math.min(currentPage * itemsPerPage, getAllSubCategoryData.length)} of {getAllSubCategoryData.length} entries
        </span>
        <Stack spacing={2}>
          <Pagination
            count={totalPages}
