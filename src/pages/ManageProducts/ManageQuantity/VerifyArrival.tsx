...
export const VerifyArrival = () => {
  ...
  useEffect(() => {
    const fetchData = async () => {
      try {
        ...
        // Set mock product data (replace with actual API call)
      } catch (error) {
  ...
  const handleUpdate = async (e: React.FormEvent) => {
    ...
    if (!formData.instock_date) {
      toast.error("Stock date is required", {
        position: "top-right",
        autoClose: 3000,
        style: { zIndex: 9999999999, marginTop: "4rem" },
      });
    ...
    if (!formData.instock_qty) {
      toast.error("Qty Received is required", {
        position: "top-right",
        autoClose: 3000,
        style: { zIndex: 9999999999, marginTop: "4rem" },
      });
    ...
    try {
      ...
      await verifyProcedure(pid, payload);
      toast.success("Procedure Verify successfully", {
        position: "top-right",
        autoClose: 3000,
        style: { zIndex: 9999999999, marginTop: "4rem" },
      });
      ...
      <ComponentCard title="Details">
        ...
        >
          ...
          <div className="grid grid-cols-2 gap-x-4 gap-y-6">
            ...
            <div className="grid grid-cols-1 gap-x-4 gap-y-6">
              <div className="flex flex-col w-full">
                <label htmlFor="instock_date" className="font-medium mb-2">
                  In Stock Date
                </label>
                <Flatpickr
                  options={{ dateFormat: "Y-m-d" }}
                  ...
                  onChange={(date) => handleDateChange(date, "instock_date")}
                  disabled={loading}
                  className="w-full border p-2 rounded"
                  placeholder="Select Date"
                />
